...
export default function AdminDashboard() {
    ...
    const getStatusStyle = (index: number) => {
        const styles = [
            { bg: 'bg-cyan-100 dark:bg-cyan-500/20', text: 'text-cyan-700 dark:text-cyan-400', label: 'In Progress' },
            { bg: 'bg-red-100 dark:bg-red-500/20', text: 'text-red-700 dark:text-red-400', label: 'Critical' },
            { bg: 'bg-emerald-100 dark:bg-emerald-500/20', text: 'text-emerald-700 dark:text-emerald-400', label: 'Completed' },
            { bg: 'bg-amber-100 dark:bg-amber-500/20', text: 'text-amber-700 dark:text-amber-400', label: 'Pending' }
        ];
        ...
        <div className="w-full space-y-6 pb-12 font-sans text-slate-900 dark:text-white min-h-screen">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-2">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1">StayTrack Boarding House operations are fully active.</p>
                </div>
                <div className="flex items-center gap-3">
                    <button className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-zinc-800 rounded-lg text-sm font-medium text-slate-700 dark:text-zinc-300 shadow-[0_2px_4px_rgba(0,0,0,0.02)] hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                        <Calendar className="w-4 h-4 text-slate-500 dark:text-zinc-400" />
                        {currentDate}
                    </button>
                    <button className="p-2 bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-zinc-800 rounded-lg text-slate-500 dark:text-zinc-400 shadow-[0_2px_4px_rgba(0,0,0,0.02)] hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                {/* Occupancy Rate */}
                <div className="bg-white dark:bg-[#0a0a0a] rounded-2xl border border-slate-100 dark:border-zinc-800 p-5 shadow-[0_4px_12px_rgba(0,0,0,0.03)] dark:shadow-none">
                ...
                <div className="bg-white dark:bg-[#0a0a0a] rounded-2xl border border-slate-100 dark:border-zinc-800 p-5 shadow-[0_4px_12px_rgba(0,0,0,0.03)] dark:shadow-none">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-medium text-slate-500 dark:text-zinc-400">Total Revenue ({new Date().toLocaleString('default', { month: 'short' })})</h3>
                        <div className="w-8 h-8 rounded-full bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600 dark:text-emerald-400">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Chart Section */}
                <div className="lg:col-span-2 bg-white dark:bg-[#0a0a0a] rounded-2xl border border-slate-100 dark:border-zinc-800 p-6 shadow-[0_4px_12px_rgba(0,0,0,0.03)] dark:shadow-none">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barGap={4} barSize={14}>
                                ...
                                <Tooltip 
                                    ...
                                    contentStyle={{ borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', backgroundColor: '#0a0a0a', color: '#fff' }}
                                    itemStyle={{ color: '#fff' }}
                                />
                ...
                {/* Quick Actions */}
                <div className="bg-white dark:bg-[#0a0a0a] rounded-2xl border border-slate-100 dark:border-zinc-800 p-6 shadow-[0_4px_12px_rgba(0,0,0,0.03)] dark:shadow-none flex flex-col">
                    <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-6">Quick Actions</h2>
                    <div className="space-y-3 flex-1">
                        <Link href="/admin/rooms" className="flex items-center gap-3 w-full p-3.5 bg-slate-50/50 dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-zinc-300 rounded-xl transition-colors">
                            <Plus className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                            <span className="font-semibold text-sm">Add New Room</span>
                        </Link>
                        <Link href="/admin/tenants" className="flex items-center gap-3 w-full p-3.5 bg-slate-50/50 dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-zinc-300 rounded-xl transition-colors">
                            <UserPlus className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                            <span className="font-semibold text-sm">Register Tenant</span>
                        </Link>
                        <Link href="/admin/billing" className="flex items-center gap-3 w-full p-3.5 bg-slate-50/50 dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-zinc-300 rounded-xl transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Recent Payments */}
                <div className="bg-white dark:bg-[#0a0a0a] rounded-2xl border border-slate-100 dark:border-zinc-800 p-6 shadow-[0_4px_12px_rgba(0,0,0,0.03)] dark:shadow-none">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-lg font-bold text-slate-800 dark:text-white">Recent Payments</h2>
                        <Link href="/admin/billing" className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300">View All</Link>
                    </div>
                        ...
                        {stats.recentPayments.slice(0, 3).map((payment, i) => {
                            ...
                            return (
                                <div key={payment.id} className="flex items-center justify-between">
                                    <div>
                                        <p className="font-bold text-slate-800 dark:text-white text-sm">{payment.tenant_name}</p>
                                        <p className="text-xs font-medium text-slate-400 dark:text-zinc-500 mt-0.5">Room 20{i + 1}-A • via {method}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold text-emerald-600 dark:text-emerald-400 text-sm">₱{Number(payment.amount_paid).toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                                        <p className="text-[11px] font-bold text-slate-400 dark:text-zinc-500 mt-0.5 uppercase tracking-wider">{new Date(payment.payment_date).toLocaleString('default', { month: 'short', day: 'numeric' })}</p>
                                    </div>
                                </div>
                            );
                        })}
                        {stats.recentPayments.length === 0 && (
                            <p className="text-sm text-slate-500 dark:text-zinc-500 text-center py-4">No recent payments.</p>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-[#0a0a0a] rounded-2xl border border-slate-100 dark:border-zinc-800 p-6 shadow-[0_4px_12px_rgba(0,0,0,0.03)] dark:shadow-none">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-lg font-bold text-slate-800 dark:text-white">Recent Maintenance</h2>
                        <Link href="/admin/requests" className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300">View All</Link>
                    </div>
                    <div className="space-y-5">
                        {stats.recentRequests.slice(0, 3).map((request, i) => {
                            ...
                            return (
                                <div key={request.id} className="flex items-center justify-between">
                                    <div>
                                        <p className="font-bold text-slate-800 dark:text-white text-sm">{request.title}</p>
                                        <p className="text-xs font-medium text-slate-400 dark:text-zinc-500 mt-0.5">Room 10{i + 1}</p>
                                    </div>
                        ...
                        {stats.recentRequests.length === 0 && (
                            <p className="text-sm text-slate-500 dark:text-zinc-500 text-center py-4">No recent maintenance requests.</p>
                        )}
                    </div>
                </div>
            </div>
